...
public interface LoggerDao {
    ...
    @Query("SELECT * FROM logger_table WHERE tag = :tag AND msg LIKE '%' || :filterValue || '%' ORDER BY date ASC")
    LiveData<List<Logger>> getAllByTagAndFilterValueLikeAsc(String filterValue,String tag);

    @Query("SELECT * FROM logger_table WHERE tag = :tag AND msg LIKE '%' || :filterValue || '%' ORDER BY date DESC")
...
//tag.equals(Logger.TRACE) ? getAllLogs() :
}
